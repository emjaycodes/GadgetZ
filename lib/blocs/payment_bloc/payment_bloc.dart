...
class PaymentBloc extends Bloc<PaymentEvent, PaymentState> {
  ...
  PaymentBloc(this.context) : super(PaymentInitialState()) {
    String getLocationFromBlocState() {
      ...
      if (locationState is LocationLoadedState) {
        return locationState
            .address; // Adjust this based on your actual LocationState
      }
    ...
    on<CardPaymentEvent>((event, emit) async {
      ...
      final PaystackPayment payment =
          PaystackPayment(event.ctx, event.email, event.price);
      try {
    ...
    // on<PaymentDoneEvent>((event, emit) async {
    //   // Send push notification for order in route
    //   final user = FirebaseAuth.instance.currentUser;
    //   final userEmail = user?.email;
    //   if (userEmail != null) {
    //     final location =
    //         getLocationFromBlocState(); // Get the location from your LocationBloc
    //     final pushNotification = FirebasePushNotification();
    //     // await pushNotification.sendOrderInRouteNotification(
    //     //     userEmail, location);
    //   }
    // });
  }
